...
void MatMul::operate(Tensor &aTensor1, Tensor &aTensor2,
    ...
    if ((dim_1 != dim_2) || shape_1[dim_1 - 1] != shape_2[dim_2 - 2]) {
        std::cout << dim_1 << " != " << dim_2 << " || " << shape_1[dim_1 - 1]
                  << " != " << shape_2[dim_2 - 2] << std::endl;
        throw std::logic_error("Tensors are not same shape");
    }

    std::vector<int> batch(dim_1 -
                           3); // taking all dims that arent the last two
    ...
    do {
        ...
        */
        for (size_t row = 0; row < shape_1[dim_1 - 2];
             row++) { // shape 1's row dim
            for (size_t col = 0; col < shape_2[dim_2 - 1];
                 col++) { // shape 2's col dim
                ...
                for (size_t shared = 0; shared < shape_1[dim_1 - 1]; shared++) {
                    ...
                    // since broadcasting not implemented, indexBatch should
                    // return the same thing for both calls std::cout << dim_1
                    // -2 << "  " << aTensor1.strides.size() << std::endl;
                    index_tensor_1 = row * aTensor1.strides[dim_1 - 2] +
                                     shared * aTensor1.strides[dim_1 - 1] +
                                     indexBatch(batch, aTensor1.strides);

                    index_tensor_2 = col * aTensor2.strides[dim_1 - 1] +
                                     shared * aTensor2.strides[dim_1 - 2] +
                                     indexBatch(batch, aTensor2.strides);

                    sum += aTensor1.getDataA()[index_tensor_1] *
                           aTensor2.getDataA()[index_tensor_2];
