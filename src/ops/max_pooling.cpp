...
void scanTensor(Tensor &tensor, std::vector<int>& kernel_shape, Tensor &result_location){
...
void MaxPooling::operate(Tensor &aTensor1, Tensor &aTensor2, Tensor &result_location) //aTensor2 is the kernel
{
    ...
    int out_height = floor(( tensor_height + pad_top + pad_bottom - kernel_height) / stride_vert) + 1;
    int out_width = floor(( tensor_width + pad_left + pad_right - kernel_width) / stride_horz) + 1;
